...
class Collections {
    ...
    constructor( config ) {
        this._collections = {
            layouts:    new Collection('layouts',  { output: false }),
            includes:   new Collection('includes', { output: false }),
            pages:      new Collection('pages',    { output: true }),
            posts:      new Collection('posts',    { output: true }),
            // SASS imports - don't output directly.
            sass:       new Collection('sass',     { output: false })
        };
    ...
    getCollectionForPath( path ) {
        ...
        if( path[0] == '_' ) {
            ...
            if( !collection ) {
                ...
                collection = new Collection( name, { output: true });
