...
class Collection {
    ...
    addStaticFile( path ) {
        ...
        this._static_files.add({ path, basename, extname, name, target_path });
    }
...
class Document extends HeckleDrop {
    ...
    get html() {
        ...
        const path = Path.join( this.$build.source, this._path );
    ...
    async excerpt( context ) {
        ...
        const {
            ...
            data: { excerpt_separator: sep }
        } = this;

        const html = await this.content( context );
        ...
        // If no excerpt separator then return just the first paragraph.
        if( !sep ) {
            const excerpt = $ps.first().html();
            return excerpt;
        }

        // Iterate over each paragraph up to the excerpt separator, if any.
        let excerpt = '', separatorFound = false;
        for( const p of $ps ) {
            const $p = $( p );
            if( $p.text().trim() === sep ) {
                separatorFound = true;
        ...
        // If no separator found then revert to default behaviour.
        if( !separatorFound ) {
            excerpt = $ps.first().html();
        }
