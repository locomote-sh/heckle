...
async function build( source, target, opts = {}, exts, files ) {
    ...
    // Resolve build extensions.
    if( !exts ) {
        exts = {};
    }
    if( config.extensions ) {
        const extPath = Path.resolve( Path.join( source, config.extensions ) );
        try {
            exts = Object.assign( exts, require( extPath ) );
        }
        catch( e ) {
...
async function loadSiteConfig( source, opts = {} ) {
    ...
    if( config ) {
        // Copy options to the config.
        config.opts = Object.assign( {}, opts );
    ...
    if( !config ) {
        ...
        const yaml = await readFile( configPath );
        if( yaml !== null ) {
            config = YAML.safeLoad( yaml ) || {};
        }
    }
